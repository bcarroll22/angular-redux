...
### `<provider store>`
The provider component is registered internally by ng-component-redux. It accepts one binding, and that is the Redux store ([createStore](http://redux.js.org/docs/api/createStore.html)). Currently, this package does not support passing the store to each connected component, but it is planned for the 1.0.0 release.
